 ...
 * Redux AsyncThunk to call to get users from the server
 */
export const getUserLogin = createAsyncThunk('users/getUserLogin', async (userToLogin) => {
    const response = await fetch(
        `http://localhost:9000/users/login/${userToLogin.email}/${userToLogin.password}`
    )   
    if (response.ok) {
        const data = await response.json();
        return data
